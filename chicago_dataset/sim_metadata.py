...
class ChargingAlgoParams(object):
    ...
    radius_dispatch_min = 10
    relocation_radius_min = 5
    fake_charging_during_day = True


class ChargingAlgo(Enum):
    CHARGE_ALL_IDLE_CARS = auto()
    CHARGE_ALL_IDLE_CARS_AT_NIGHT = auto()
    CHARGE_ALL_IDLE_CARS_RADIUS_DISPATCH = auto()
    CHARGE_ALL_IDLE_CARS_AT_NIGHT_WITH_RELOCATION = auto()
    CAN_WITH_IC_AT_NIGHT = auto() # Charge at night with interrupt charging at night
...
class DatasetParams(object):
    percent_of_trips_filtered = 0.9
    percentile_lat_lon = 99.9
    longitude_range_min = -87.75
    latitude_range_min = 41.8
    longitude_range_max = -87.55
    latitude_range_max = 42
    delta_latitude = 1
    delta_longitude = 1
