...
class DataInput:
    ...
    def real_life_dataset(self,
        ...
        if os.path.isfile(sample_data_path) and SimMetaData.test is True:
            ...
            if dataset_source == Dataset.NYTAXI.value:
                ...
                df_filtered.loc[:, "pickup_datetime"] += pd.to_timedelta(
                    np.random.randint(0, 15 * 60, len(df_filtered)), unit="s"
                )
            ...
            if self.percentile_lat_lon is not None:
                ...
                delta_lat_lon_perc = 5
                while abs(DatasetParams.latitude_range_max - DatasetParams.latitude_range_min) > DatasetParams.delta_latitude:
                    warnings.warn(f"The max and min latitudes are too far away from each other, "
                                "reducing the percentile by {delta_lat_lon_perc}%.")
                    lower_percentile_lat_lon_latitude = lower_percentile_lat_lon_latitude + delta_lat_lon_perc / 2
                    upper_percentile_lat_lon_latitude = upper_percentile_lat_lon_latitude - delta_lat_lon_perc / 2
                    DatasetParams.latitude_range_min, DatasetParams.latitude_range_max = np.percentile(
                        df_filtered["pickup_latitude"],
                ...
                while abs(DatasetParams.longitude_range_max - DatasetParams.longitude_range_min) > DatasetParams.delta_longitude:
                    warnings.warn(f"The max and min longitudes are too far away from each other, "
                                "reducing the percentile by {delta_lat_lon_perc}%.")
                    lower_percentile_lat_lon_longitude = lower_percentile_lat_lon_longitude + delta_lat_lon_perc / 2
                    upper_percentile_lat_lon_longitude = upper_percentile_lat_lon_longitude - delta_lat_lon_perc / 2
                    DatasetParams.longitude_range_min, DatasetParams.longitude_range_max = np.percentile(
                        df_filtered["pickup_longitude"],
        ...
        if DatasetParams.uniform_locations is True:
            ...
            df_output["dropoff_datetime"] = df_output["pickup_datetime"] + pd.to_timedelta(df_output["trip_time_min"], 'm')
