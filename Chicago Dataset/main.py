...
from sim_metadata import SimMetaData, TripState, MatchingAlgo, ChargingAlgoParams, Dataset, DatasetParams, \
    AdaptivePowerOfDParams, ChargingAlgo, Initialize, AvailableCarsForMatching, DistFunc, PickupThresholdType, PickupThresholdMatchingParams
from real_life_data_input import DataInput
from ev_database import ElectricVehicleDatabase
from algo_database import AlgorithmDatabase
import argparse
from distutils.util import strtobool
from mpl_toolkits.basemap import Basemap
from utils import is_not_number
...
def run_simulation(
    ...
    if match_and_charge_algo is None:
        raise ValueError("Please specify a matching and charging algorithm - refer to algo_database.py -  pick from the already existing ones or make your own")
    obj_match_and_charge_algo = AlgorithmDatabase()
    info_match_and_charge_algo = obj_match_and_charge_algo.get_algorithm(match_and_charge_algo)
    matching_algo = info_match_and_charge_algo.matching_algo
    charging_algo = info_match_and_charge_algo.charging_algo
    available_cars_for_matching = info_match_and_charge_algo.available_cars_for_matching
    print(f"Algorithm: {info_match_and_charge_algo.name}")
    if is_not_number(pickup_threshold_min):
        raise ValueError(f"The user must specify values of d and pickup_threshold_min for {info_match_and_charge_algo.name}")
    if match_and_charge_algo in ["POD", "CAN_POD", "CAN_R_POD", "CAN_R_POD_N"]: # Require d under POD
        if is_not_number(d):
            raise ValueError(f"The user must specify values of d and pickup_threshold_min for {info_match_and_charge_algo.name}")
    if match_and_charge_algo in ["CAD", "CAN_CAD"]:
        d = 0 # Set d = 0 whenever we use Closest Available Dispatch for better results folder naming

    if match_and_charge_algo == "CAN_R_POD_N":
        SimMetaData.d_during_the_night = d
        print(f"Under {info_match_and_charge_algo.name}, the value of d is only used for the nighttime - during the day, d is set to be 1 by default")

    # Initialize all supercharging stations
    list_chargers = []
    # basemap = Basemap(projection='merc', llcrnrlat=41.6, urcrnrlat=42, llcrnrlon=-87.9, urcrnrlon=-87.5, resolution='h')
    ...
    top_level_dir = os.path.join(results_folder, f"{d}_{pickup_threshold_min}_{match_and_charge_algo}_{ev_model}_ckw_{charge_rate_kw}_ev_{n_cars}_nc_{n_chargers}_f_{perc_trip_filter}_{curr_date_and_time}")
    ...
    kpi = pd.DataFrame({
        ...
        "algo": match_and_charge_algo,
        "d": d,
        "pickup_threshold_min": pickup_threshold_min,
        "fleet_size": n_cars,
        "n_chargers": n_chargers,
        "perc_trip_filter": perc_trip_filter,
    ...
    if SimMetaData.save_results:
        ...
        # Saving initial distribution of EVs
        spatial_data_file = os.path.join(demand_curve_dir, "initial_ev_loc.csv")
        pd.DataFrame({
            "lat": [car.lat for car in car_tracker],
            "lon": [car.lon for car in car_tracker]
        ...
        if match_and_charge_algo in ["CAN_R_POD_N", "CAN_R_POD"]:
        ...
        ax1.plot(current_min_list, num_trips_in_progress_list, color="#7B3F00", linewidth=2)
        # plt.title("Demand Stackplot with SOC overlaid")
        demand_curve_plot_file = os.path.join(plot_dir, "demand_curve_stackplot.png")
        plt.savefig(demand_curve_plot_file, bbox_inches='tight')
        plt.clf()
        ...
        for i in range(len(bins) - 1):
            ...
            if len(indices) == 0:
                ...
                mean_pickup_time = np.mean([list_pickup_time_min[idx] for idx in indices])
                std_pickup_time = np.std([list_pickup_time_min[idx] for idx in indices])
                pickup_datapoints = len(indices)
            # Calculate mean of corresponding values in list_pickup_time
            list_mean_pickup_times.append(mean_pickup_time)
            list_std_pickup_times.append(std_pickup_time)
            list_pickup_datapoints.append(pickup_datapoints)
        ...
        file_name = ["drive_to_charger_time_vs_available_chargers", "drive_to_charger_time_vs_available_posts", "drive_to_charger_time_vs_available_posts_with_driving_cars"]
        count = 0
        for car in car_tracker:
            ...
            list_n_available_posts_with_driving_cars.extend([posts - ChargingAlgoParams.n_cars_driving_to_charger_discounter * to_charger for posts, to_charger in zip(car.list_n_available_posts, car.list_n_cars_driving_to_charger)])
        for x_data in [list_n_available_chargers_to_match, list_n_available_posts, list_n_available_posts_with_driving_cars]:
            ...
            for i in range(len(bins) - 1):
                ...
                if len(indices) == 0:
                    mean_drive_to_charger_time_min = 0
                    std_drive_to_charger_time_min = 0
                    drive_to_charger_datapoints = 0
                else:
                    mean_drive_to_charger_time_min = np.mean([list_drive_to_charger_time_min[idx] for idx in indices])
                    std_drive_to_charger_time_min = np.std([list_drive_to_charger_time_min[idx] for idx in indices])
                    drive_to_charger_datapoints = len(indices)
                # Calculate mean of corresponding values in list_pickup_time
                list_mean_drive_to_charger_time_min.append(mean_drive_to_charger_time_min)
                list_std_drive_to_charger_time_min.append(std_drive_to_charger_time_min)
                list_drive_to_charger_datapoints.append(drive_to_charger_datapoints)

            plt.errorbar(bins[0:len(bins) - 1], list_mean_drive_to_charger_time_min, list_std_drive_to_charger_time_min, linestyle='None', marker='^')
        ...
        plt.title('Scatter Plot of Latitude and Longitude in Chicago')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-algo', '--matching_and_charging_algo', type=str, default="POD")
    parser.add_argument('-d', '--power_of_d', type=float, default=1)
    parser.add_argument('-ad', '--adaptive_d', type=str, default="False")  # Accepts string values
